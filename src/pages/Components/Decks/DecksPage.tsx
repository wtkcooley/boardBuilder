...
class DecksPage extends React.Component<Props, State> {
    ...
    async handleAddToBuild(e: any, deck: Deck) {
        e.preventDefault();
    ...
    render() {
        const deck = node.filter((item: any) => {
            return item.category === "deck"
        })
        let items = deck.map((deck: Deck) => {
            const backgroundImage: CSSProperties = {
                backgroundImage: `url(${deck.image})`
            }
            return (
                <IonItem className="component" routerDirection="back" onClick={e => this.handleItemClick(e, deck)} button={true}>
