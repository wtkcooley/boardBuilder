...
import {Griptape} from "../../../metadata/itemInfo";
...
class GriptapePage extends React.Component<Props, State> {
    ...
    async handleAddToBuild(e: any, griptape: Griptape) {
        e.preventDefault();
    ...
    render() {
        const griptape = node.filter((item: any) => {
            return item.category === "griptape"
        })
        let items = griptape.map((griptape: Griptape) => {
            const backgroundImage: CSSProperties = {
                backgroundImage: `url(${griptape.image})`
            }
            return (
                <IonItem className="component" routerDirection="back" onClick={e => this.handleItemClick(e, griptape)} button={true}>
                ...
                <IonHeader>
                    <IonToolbar>
                        <div className="toolbar-container">
                            <IonTitle className="md title-default hydrated">Griptapes</IonTitle>
