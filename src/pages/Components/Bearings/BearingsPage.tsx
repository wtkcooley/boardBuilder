...
class BearingsPage extends React.Component<Props, State> {
    ...
    async handleAddToBuild(e: any, bearings: Bearings) {
        e.preventDefault();
    ...
    render() {
        ...
        let items = bearings.map((bearings: Bearings) => {
            const backgroundImage: CSSProperties = {
                backgroundImage: `url(${bearings.image})`
            }
            return (
                <IonItem className="component" routerDirection="back" onClick={e => this.handleItemClick(e, bearings)} button={true}>
